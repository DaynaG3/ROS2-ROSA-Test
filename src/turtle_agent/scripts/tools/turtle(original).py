...
def publish_twist_to_cmd_vel(
    node: Node, name: str, velocity: float, lateral: float, angle: float, steps: int = 1
):
    name = name.replace("/", "")
    ...
    in_bounds, message = will_be_within_bounds(node, name, velocity, lateral, angle, duration=steps)
    if not in_bounds:
        return message
    ...
    try:
        ...
        current_pose = get_turtle_pose(node, [name])
        return (
